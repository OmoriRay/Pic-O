...
按 `Ctrl+K` 会在窗口中上方弹出快速搜索浮层。默认输入图片序号并按 Enter 跳转，也可以在设置中改为按文件名关键词查找；按 Esc 可随时收起。按住 Ctrl 后拖动搜索框可以调整位置，退出并重新打开 Pixora 会保留该位置；用快速搜索快捷键关闭后再打开会恢复默认位置。按文件名搜索且右侧缩略图栏可见时，缩略图栏会实时只显示匹配项；没有匹配项时保留右侧上一次有效内容和滚动位置，不显示空白栏。缩略图栏隐藏时不会为搜索额外构建或加载预览。
...
- 新增可拖动并记住位置的快速搜索浮层，支持按序号或文件名查找、匹配缩略图预览和明确的快捷键/文字输入状态。
- 优化单列/双列缩略图布局、选中边框和固定间距；快速滚动大目录时会取消过期视口任务，停稳后只加载当前区域。
- 当前目录中的新增、删除、修改和重命名可增量更新，监听异常或超大批次时自动回退完整扫描。
- 设置、快捷键、收藏和批量压缩配置改为原子保存，并在主文件损坏时尝试从备份恢复。
- 改善 Per-Monitor V2 DPI、键盘导航、动态状态无障碍信息，以及设置页分类、常驻搜索和缓存操作体验。
